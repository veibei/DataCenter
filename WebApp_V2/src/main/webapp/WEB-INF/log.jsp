...
<!-- userJs -->
<script type="text/javascript" src="js/logPageFunction/logPage.js"></script>
							...
							<div class="box-content buttons" style="min-height: 550px ;height:auto">
										...
										<div id="DataTables_Table_0_length" class="dataTables_length">
											<label> 每页&nbsp<select id="perPageCountSelect" name="DataTables_Table_0_length"
												size="1" aria-controls="DataTables_Table_0"  onchange="changePerPage();">
													...
													<option value="10"  <s:if test="#session.perPageCount==10">selected="selected"</s:if> >10</option>
													<option value="25" <s:if test="#session.perPageCount==25">selected="selected"</s:if>>25</option>
													<option value="50" <s:if test="#session.perPageCount==50">selected="selected"</s:if>>50</option>
													<option value="100" <s:if test="#session.perPageCount==100">selected="selected"</s:if>>100</option>
											</select> 条记录
											</label>
										</div>
									</div>
								...
								<table
									...
									<tbody>
										...
										<s:iterator value="#session.page" id="log" status="st">
											<tr>
												<td class="center"><s:property value="#log.dateString" /></td>
												<td class="center"><s:property value="#log.message" /></td>
												<td class="center"><span
													class="label-success label label-default"><s:property
															value="#log.log_level" /></span></td>
											</tr>
								...
								<s:property value="#session.rowsCount" />
								条记录&nbsp 共
								...
								<ul class="pagination pagination-centered">
									...
									<s:if test="#session.currentPage>1">
										<li><a href="logSubPage?pageIndex=<s:property	value="#session.currentPage-1" />">前一页</a></li>
									</s:if>
									<s:iterator value="#session.listBtn" id="value" status="st">
										<s:if test="#value==#session.currentPage">
											<li class="active"> <a href="logSubPage?pageIndex=<s:property	value="#value" />"><s:property	value="#value" /></a></li>	
										</s:if>
											
										<s:else>
											<li><a href="logSubPage?pageIndex=<s:property	value="#value" />"><s:property value="#value" /></a></li>
										</s:else>
									...
									<!-- 显示后一页按钮 -->
									<s:if test="#session.currentPage<#session.pageCount">
										<li><a href="logSubPage?pageIndex=<s:property	value="#session.currentPage+1" />">后一页</a></li>
									</s:if>
